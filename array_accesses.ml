...
let run_timed s arr_size f =
  ...
  let duration_micros = duration *. 1000000.0 in
  let duration_per_element = duration_micros /. (float_of_int arr_size) in
  Printf.printf "%s %d %d %d\n%!" s arr_size (int_of_float duration_micros) (int_of_float duration_per_element)
...
let atomic_arrayt_sequential arr_size =
  let arr = Array.make arr_size (Atomic.make 0) in
  Gc.minor ();
  for _ = 0 to iterations-1 do
    for i = 0 to arr_size-1 do
